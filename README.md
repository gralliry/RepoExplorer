...
`F5` 刷新、`Ctrl+A` 全选、`Ctrl+C / Ctrl+X / Ctrl+V` 复制 / 剪切 / 粘贴、
`Alt+←/→` 后退 / 前进、`Esc` 取消。
...
| 分区 | 内容 |
|---|---|
| 账号 | 用 GitHub 登录 / 手动填 Token / 退出登录 |
| 下载 | 下载目录（下载选中文件时保存到哪里） |
| 打开文件 | 临时文件目录（双击文件时先下载到这里），带「打开目录」按钮 |

未登录时「打开仓库」弹窗里会提示「未登录，只能打开公开仓库」，并有个按钮直接跳到设置。
...
- 移动到别的目录，支持整个文件夹
- 复制 / 剪切 / 粘贴（`Ctrl+C` / `Ctrl+X` / `Ctrl+V`，右键菜单里也有）。
  **复制不会重新上传内容**：新路径直接复用原来的 blob，复制 10 MB 和复制 10 字节一样快。
  同目录粘贴会自动改名成 `xxx - 副本`，绝不覆盖已有文件；剪切过的项目在粘贴前显示为半透明
- 删除文件 / 文件夹
...
0. 点右上角齿轮 **⚙ 设置 → 账号 → 用 GitHub 登录**。
   **读公开仓库不需要登录，任何写操作都需要。** 见下方「GitHub 认证」。
1. 点「打开仓库」弹出选择器：
...
双击文件（或右键「打开（系统默认程序）」）会：

1. 把文件下载到 `%TEMP%\RepoExplorer\<仓库>\<分支>\<原路径>`
2. 调用 Windows 的 `ShellExecute`，交给系统关联的程序打开（和资源管理器双击一样）

**程序自己不实现预览和编辑。** Markdown、图片、PDF、代码……都交给系统里你装的程序去处理。

要注意的是：**本地那份改动不会写回仓库**。它是临时副本，仓库才是唯一事实来源。
如果要修改仓库里的文件，用「上传」把改好的文件覆盖上去。

临时文件会一直留在 `%TEMP%\RepoExplorer` 下（重复打开同一文件会覆盖），可以随时整个删掉。
...
| `OpenFile(repo, ref, path)` | 下载到临时目录并用系统默认程序打开 |
| `ReadFile(repo, ref, path)` | 读取文本文件内容（测试用） |
...
- 程序本身不预览、不编辑文件。双击文件只是把它下载到
  `%TEMP%\RepoExplorer\<仓库>\<分支>\<原路径>` 然后交给系统打开，
  **本地改动不会写回仓库** —— 要改内容请用「上传」覆盖。
